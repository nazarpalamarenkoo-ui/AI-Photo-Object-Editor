...
def _make_app(db_session, mock_redis=None):
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_get_detection_by_bbox_not_found(
    db_session, sample_user, sample_image, sample_image_version, mock_redis_cache
):
    app = _make_app(db_session, mock_redis_cache)
