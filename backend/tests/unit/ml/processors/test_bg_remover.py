...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_remove_and_resize(background_remover, test_image_bytes):
    """Test remove_and_resize basic functionality"""

    async def fake_remove(*args, **kwargs):
        img = Image.open(BytesIO(test_image_bytes)).convert("RGBA")
        buf = BytesIO()
        img.save(buf, format="PNG")
        return buf.getvalue()

    background_remover.remove_background = fake_remove

    out = await background_remover.remove_and_resize(
        image_bytes=test_image_bytes,
        target_size=(128, 128)
    )
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_remove_and_resize_alpha_processed(background_remover, test_image_bytes):
    """Test that alpha channel is processed (not empty)"""
    ...
    img = np.array(Image.open(BytesIO(out)))
